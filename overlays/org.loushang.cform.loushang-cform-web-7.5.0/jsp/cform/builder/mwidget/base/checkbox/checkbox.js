...
(function($) {
	var overrides = {
		// 创建组件模板
		tpl : "<div cf_widgetId='{widgetId}' class='phone_widget phone_widget_checkbox'>" +
		...
		drop : function(e, target) {
			...
			showWindow(L.getLocaleMessage("cf.widget.checkbox", "复选按钮"), "mwidget/base/checkbox/checkboxattr.jsp", 400, 400,
			...
			function handleReturn(result) {
				...
				var $labelTpl = new $.XTemplate(self.checkboxTpl);
				...
				$.each(optionArr, function(i,option) {
					...
					$widgetWrapTpl.find(".checkboxContainer").append($filed);
				});
			}
		},
		...
		dblClick : function(e, target) {
			//获得所有单选按钮
			var $tgInputs = $(target).parents(".phone_widget").find(":checkbox");
			//获得所有选项组成的字符串
			...
			function handleReturn(result){
				...
				// 将当前域做为模板
				var checkbox = $(target).parents(".phone_widget_checkbox").find(":checkbox")[0];
				var $labelTpl =$(checkbox).parent().clone();
				//清空之前的选项
				var $checkboxContainer = $(checkbox).parents(".checkboxContainer");
				...
				$.each(optionArr, function(i,option){
					...
					$checkboxContainer.append($filed);
				});
			}			
		}		
	};
