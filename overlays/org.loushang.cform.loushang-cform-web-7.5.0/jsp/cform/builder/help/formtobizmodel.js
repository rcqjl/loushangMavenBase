...
function initFormTree() {
	...
	// 区域节点
	var zoneArray = $form.find('[cf_elementType=zone][class!=cfEachFormGroup]');
	...
	$.each(fieldArray, function() {
		...
		if(!obj[fieldId]){
			...
			var modelName = $(this).attr("cf_modelName");

			if (modelItemId) {
				var modelItemJson = {};
				modelItemJson.fieldId = modelItemId;
				if(modelName){
					modelItemJson.fieldName = modelName + '['+modelItemName+']';
				}else{
...
function addHoverDomHandler(treeId, treeNode) {
	...
	if (treeNode.type == Constants.FormTree.BizModelItem) {
		...
		// 所以将":"替换为"_"，保存为新的属性
		treeNode.delBtn = $.trim(treeNode.fieldId.replace(":", "_"));
	}
	if (treeNode.type != Constants.FormTree.BizModelItem
		...
		if (btn) {
			btn.bind("click", function(e) {
				showConfirm(L.getLocaleMessage("cf.bdr.confirmdeletemapping", "确定删除映射吗？"), function(){
					...
					$field.removeAttr('cf_modelName');
					$field.removeAttr('cf_modelId');
					$field.removeAttr('cf_parentModelId');
					
					// 获取父区域
					var zoneEles = $field.parents("fieldset");
					if (zoneEles) {
						var $zoneEle = zoneEles.eq(0);
						// 查找父区域内所有关联模型项的域
						var $fs = $zoneEle.find("[cf_modelItemId]");
						if (!$fs.length) {
							// 如果域没有关联，则删除父区域关联的模型
							$zoneEle.removeAttr("cf_modelId");
							$zoneEle.removeAttr("cf_modelName");
						}
					}
...
function initBizModelTree() {
	...
	var treeSetting = {
		...
		async : {
			...
			otherParam : [ "mainModelId", $form.attr("cf_modelId") ]
		},
		callback : {
...
function beforeDrag(treeId, treeNodes) {
	for ( var i = 0, l = treeNodes.length; i < l; i++) {
		...
		// 将业务模型项所属业务模型的父业务模型同时保存
		treeNodes[i].parentModel = treeNodes[i].getParentNode().parentModelId; 
	}
	return true;
}
...
function beforeDrop(treeId, treeNodes, targetNode, moveType) {
	// 目标节点不是域节点
	if (!targetNode || targetNode.type != Constants.FormTree.FieldNode) {
		return false;
	}
	...
	if ($field) {
		if (treeNodes.length > 0) {
			if (targetNode.children && targetNode.children.length >= 1) {
				cfalert(L.getLocaleMessage("cf.bdr.onefieldbizmodelmapping", "一个域只能映射一个业务模型项！"));
				return false;
			}
			...
			node.fieldName = node.parentModelName + '['+node.modelName+']';
			...
			if (zoneEles) {
				var $zoneEle = zoneEles.eq(0);
				var zoneModelId = $zoneEle.attr("cf_modelId");
				if (!zoneModelId) {
					$zoneEle.attr("cf_modelId", node.parentModelId);
					$zoneEle.attr("cf_modelName", node.parentModelName);
				}
			}
			...
			$field.attr('cf_modelName', node.parentModelName);
			$field.attr('cf_modelId', node.parentModelId);
			$field.attr('cf_parentModelId', node.parentModel);
		}
	}
