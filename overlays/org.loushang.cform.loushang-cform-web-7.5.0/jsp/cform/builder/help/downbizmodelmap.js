...
function initFormModelTree() {
	...
	$.each(fieldArray, function() {
		...
		if(!obj[modelId]){
			...
			// 下行映射
			var biz2Forms = $(this).attr("cf_biz2Form");
			if (biz2Forms) {
				var biz2FormNames = $(this).attr("cf_biz2FormName");
				var biz2FormArr = biz2Forms.split('#');
				var biz2FormNameArr = biz2FormNames.split('#');
				for (var i = 0; i < biz2FormArr.length; i++) {
					// id
					var biz2Form = biz2FormArr[i];
					// 中文名
					var biz2FormName = biz2FormNameArr[i];
					
					var ids = biz2Form.split('.');
					var modelId = ids[0];
					var modelItemId = ids[1];
					
					var names = biz2FormName.split('.');
					var modelItemName = names[1];
					
					var mapJson = {
							modelId : modelItemId,
							modelName : modelItemName,
							displayName : biz2FormName,
	...
	var zTree = $.fn.zTree.init($("#targetTreeContainer"), treeSetting, formModelTree);
	// 默认打开根节点
	...
	if (treeNode.type == Constants.FormModelTree.BizModelItemMapNode) {
		...
		if (btn) {
			btn.bind("click", function(e) {
				...
				showConfirm(L.getLocaleMessage("cf.bdr.setnewid", "请设置新表单定义ID！"), confirmFun);
			});
		}
	}
};
...
function initBizModelTree() {
	...
	var zTree = $.fn.zTree.init($("#sourceTreeContainer"), treeSetting, bizModelTree);
	// 默认打开根节点
	var nodes = zTree.getNodes();
	zTree.expandNode(nodes[0],true,false,true);
}
...
function beforeDrop(treeId, treeNodes, targetNode, moveType) {
	...
	if (treeNodes.length > 0) {
		...
		// 重复性校验
		var zTree = $.fn.zTree.getZTreeObj('targetTreeContainer');
		var node = zTree.getNodeByParam("displayName", node.displayName, targetNode);
		if (node != null) {
			cfalert(L.getLocaleMessage("cf.delete", "不能重复添加映射！"));
			return false;
		}
	}
			...
			for(var i = 0; i < itemNodes.length; i++){
				...
				if (mapNodes && mapNodes.length) {
					...
					for(var m = 0; m < mapNodes.length; m++){
						var fromModelId = mapNodes[m].parentModelId;
						var fromModelItemId = mapNodes[m].modelId;
						var fromModelName = mapNodes[m].displayName;
						mapValue += fromModelId + "." + fromModelItemId;
						mapNameValue += fromModelName;
						if (m != mapNodes.length - 1) {
							mapValue += "#";
							mapNameValue += "#";
						}
					}
					$field.attr("cf_biz2Form", mapValue);
					$field.attr("cf_biz2FormName", mapNameValue);
				} else {
					$field.removeAttr("cf_biz2Form");
					$field.removeAttr("cf_biz2FormName");
				}
			}
		}
