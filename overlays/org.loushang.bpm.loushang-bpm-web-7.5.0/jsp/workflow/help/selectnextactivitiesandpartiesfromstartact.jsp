	...
	if(parameters.length>0){
		...
		for ( var i = 0; i < keyValues.length; i++) {
			...
			if (keyValue.length == 2) {
				...
				//目前就用到了proDefUniqueId
				if (key == "procDefUniqueId" || key == "startActDefUniqueId"
						|| key == "assignmentId" || key == "actDefId"
						|| key =="selectedOrganId") {
					L5.setCP(key, value);
				}
			}
...
<body>
<model:datasets>
	<model:dataset id="actDataSet" cmd="org.loushang.workflow.client.common.help.cmd.TaskHelpCmd" method="selectNextActivitiesAndPartiesFromStartAct">
		<model:record >
