(function($) {
	...
	function initFormSize() {
		...
		$.ajax({
			type : "POST",
			async : false,
			url : WForm.webPath + "/jsp/workflow/infoprocessmodel/bizform/wfdcore.jsp",
			data : parseUrl(),
			dataType:"html",
	...
	function initOperation() {
		$("#wfd_btn_showCode").click(function() {
		...
		$("#wfd_btn_save").click(function() {
			...
			$.ajax({
				...
				url : WForm.fullWebPath + "/command/dispatcher/"
					+ "org.loushang.workflow.infoprocessmodel.bizform.cmd.FormDispatcherCmd/"
					+ "save",
				data : {"procDefUniqueId" : WForm.procDefUniqueId,
					...
					"extensionContent" : WFormModel.generateModelContent()},
				dataType:"json",
				success : function(data) {
		...
		$("#saveAsNewProcess").click(function(){
					...
					$.ajax({
						...
						success : function(data) {
							if (data && data.success) {
								...
								$("#wfd_procTitle").text(procObj.name);
			...
			var url=parent.WForm.webPath+"/jsp/workflow/infoprocessmodel/bizform/view/prompt.jsp";
			var param={
