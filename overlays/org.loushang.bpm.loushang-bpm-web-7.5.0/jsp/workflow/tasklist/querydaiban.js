	        ...
	        "columnDefs": [
	                       ...
	                       },
	                       {
		       					...
		       					"render": function(data, type, full) {
		       						...
		       						return "<a title='" + data + "' href=\"javascript:dealDaiBanTask('"+ id +"')\">"+data+"</a>";
		       					}
	                       },
	                       {
		       					"targets":3,	    		       
...
function retrieveData( sSource, aoData, fnCallback ) {
	var draw=null;
	var cmd = new L5.Command("org.loushang.workflow.tasklist.taskinfo.cmd.DaiBanTaskQueryCmd");
...
{	
	if($("input:checked").length==1 || assignmentId){
		...
		var url=L5.webPath+"/command/dispatcher/org.loushang.workflow.tasklist.forward.TaskListDispatcherCmd/daiBanTaskForward";
...
function showState(assignmentId){
	if($("input:checked").length==1 || assignmentId){
		...
		url+="?backUrl="+"jsp/workflow/tasklist/querydaiban.jsp"+"&assignmentId="+assignmentId;
		location.href=url;
	}else{
		UIAlert(L.getLocaleMessage("BPM.TASKLIST.Tip1","请选择一条记录进行处理！"));
	}
