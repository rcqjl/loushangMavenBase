...
function initTable(){
	var table = $("#formList").dtable({
			...
			"columns": [
		                  ...
		                  {"data": "procDefName"},
		                  {"data": "actDefName"},
		                  {"data": "activeActDefNames"},
		                  {"data": "activeOrganNames"},
		                  {"data": "procCreateTime"},
		                  {"data": "endTime"}
		                ],
	        "columnDefs": [
...
function retrieveData( sSource, aoData, fnCallback ) {
	var draw=null;
	var cmd = new L5.Command("org.loushang.workflow.tasklist.taskinfo.cmd.YiBanTaskQueryCmd");
...
{	
	if($("input:checked").length==1 || assignmentId){
		...
		var url=L5.webPath+"/command/dispatcher/org.loushang.workflow.tasklist.forward.TaskListDispatcherCmd/yiBanTaskForward";
...
function showState(assignmentId){
	if($("input:checked").length==1 || assignmentId){
		...
		url+="?backUrl="+"jsp/workflow/tasklist/queryyiban.jsp"+"&assignmentId="+assignmentId;
		location.href=url;
	}else{
		UIAlert(L.getLocaleMessage("BPM.TASKLIST.Tip1","请选择一条记录进行处理！"));
	}
