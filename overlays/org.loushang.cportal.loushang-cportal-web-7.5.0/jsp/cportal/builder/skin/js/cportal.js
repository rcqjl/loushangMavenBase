...
(function($) {
	if (typeof CPortal == "undefined") {
		CPortal = {
			...
			// 组件模板
			widgetTpl:'<div class="panel ue-panel cportal-widget" data_widgetId="{widgetId}" style="width:{width}">'
						...
						+'<div>'
						+'<div>'+L.getLocaleMessage("cportal.widget.author","作者")+':'
						+'{author}'
						+'</div>'
						+'<div>'+L.getLocaleMessage("cportal.widget.description","描述")+':'
						+'{desc}'
						+'</div>'
						+'</div>'
						+ '</div>'
