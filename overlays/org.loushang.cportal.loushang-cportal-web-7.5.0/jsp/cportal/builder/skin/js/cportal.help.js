...
(function($) {
	...
	CPHelp.validate = function() {
		...
		$(".cfIsRequired:visible").each(function(i, field) {
			if (!$.trim($(field).val())) {
				//showDialog("alert",$(field).attr("name")+L.getLocaleMessage("tip-nullmsg","不能为空！"),L.getLocaleMessage("cportal.message","提示信息"),L.getLocaleMessage("cportal.confirm","确定"),L.getLocaleMessage("cportal.cancel","取消"),300);
				$.dialog({
					autofocus:true,
					type : "alert",
					content : $(field).attr("name")+L.getLocaleMessage("tip-nullmsg","不能为空！")
				});
				result = false;
				return false;
			}
		...
		$(".cfIsNumber").each(function(i, field) {
			...
			if (fv && !$.isNumeric(fv)) {
				$.dialog({
					autofocus:true,
					type : "alert",
					content : $(field).attr("name")+L.getLocaleMessage("tip-numbers"," 只能输入数字！")
				});
				result = false;
				return false;
			}
